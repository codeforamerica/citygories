...
Drupal.settings.term_form = {};

Drupal.behaviors.term_form = {
  'attach': function(context, settings) {
      Drupal.settings.term_form.data = $('div.openlayers-map').data('openlayers');
      Drupal.settings.term_form.centerOnFeature();
...
/*     'div.field-name-field-activity': false, */
  ...
  Drupal.settings.term_form.geocodeAddress();
  return false;
 });
...
Drupal.settings.term_form.geocodeAddress = function (){
    var inputValue = $('div.map-instructions-container div.address-ajax input#search-map-input').val();
    console.log(inputValue);
    Drupal.settings.term_form.codeAddress(inputValue);
};

Drupal.settings.term_form.codeAddress = function(address) {
  var result =  geocoder.geocode( { 'address': address}, function(results, status) {
    if (status == google.maps.GeocoderStatus.OK) {
       Drupal.settings.term_form.mapGeocodedData(results);
    } 
    else {
...
Drupal.settings.term_form.mapGeocodedData = function(results) {
  var result = {};
  ...
  Drupal.settings.term_form.updateMapForm(result);
};
    
Drupal.settings.term_form.updateMapForm = function(result) {
  ...
  Drupal.settings.term_form.openLayersDropPoint(result);
};

Drupal.settings.term_form.openLayersDropPoint = function(result){
  // Get map data.
  var data = Drupal.settings.term_form.data;
console.log(result);
  ...
  Drupal.settings.term_form.setItem(feature);
    
  data.openlayers.setCenter(center, zoomBlockLevel, false, false);
  }
};

Drupal.settings.term_form.setItem = function(feature) {
  var data = Drupal.settings.term_form.data;
...
Drupal.settings.term_form.centerOnFeature = function() {
  var data = Drupal.settings.term_form.data;
